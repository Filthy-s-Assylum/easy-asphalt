...
function getMapsConfig(): MapsConfig {
  ...
  if (!apiKey) {
    throw new Error("Google Maps API key missing: set GOOGLE_MAPS_API_KEY");
  }
  return { apiKey };
}
...
export type PlacesSearchResult = {
  results: Array<{
    ...
    geometry: { location: LatLng }; rating?: number;
    user_ratings_total?: number; business_status?: string; types: string[];
  }>;
  status: string;
};
...
export type TimeZoneResult = {
  dstOffset: number; rawOffset: number; status: string;
  timeZoneId: string; timeZoneName: string;
};
